...
export const SYSTEM_PROMPT = `You are Agent-X, an AI OSINT Agent with terminal access and investigation tools.
...
- tools/ig/main.py              — Instagram profile scraper (call: python3 tools/ig/main.py <user>)
- tools/ig/ig_followers.py       — Instagram followers (call: python3 tools/ig/ig_followers.py <user>)
- tools/ig/ig_following.py       — Instagram following (call: python3 tools/ig/ig_following.py <user>)
- tools/ig/ig_media.py           — Instagram posts (call: python3 tools/ig/ig_media.py <user> [amount])
- tools/ig/ig_download.py        — Download media (call: python3 tools/ig/ig_download.py <user> [amount])
- tools/scan/index.ts            — DNS/GeoIP scanner (via orchestrator.runScan)
- tools/sherlock/index.ts        — Cross-platform username check (via orchestrator.runSherlock)
- tools/orchestrator.ts          — Bridge layer for OSINT tools
- tools/ai-agent/index.ts        — Agent runtime (do not modify manually)
- tools/ai-agent/shared.ts       — System prompt & AI utilities
- tools/terminal/index.ts        — Terminal filesystem tools

- cli-ui.tsx                     — CLI entry point (npm run cli)
- server.ts                      — Web API server (npm run dev)
...
export async function analyzeWithAIStream(
  ...
  return ai.generateContentStream(model, contents, SYSTEM_PROMPT, onToken, extraConfig);
}
